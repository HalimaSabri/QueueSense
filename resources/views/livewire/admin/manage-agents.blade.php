            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 border-b border-slate-100">
                        <th class="px-10 py-6 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Identity</th>
                        <th class="px-10 py-6 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Role</th>
                        <th class="px-10 py-6 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Assigned Service</th>
                        ...
                        <tr class="hover:bg-slate-50/30 transition-colors">
                            <td class="px-10 py-6">
                                <div class="flex items-center">
                                    ...
                                    <div class="ms-4">
                                        ...
                                        <div class="text-[11px] text-slate-500 font-medium">{{ $agent->email }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-10 py-6">
                            ...
                            <td class="px-10 py-6">
                                <div class="flex items-center space-x-4">
                                    <button wire:click="edit({{ $agent->id }})" class="p-2 text-slate-400 hover:text-indigo-600 transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </button>
                                    <button wire:click="delete({{ $agent->id }})" wire:confirm="Are you sure you want to delete this staff member?" class="p-2 text-slate-400 hover:text-rose-600 transition-colors">
        ...
        <div class="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm">
            <div class="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden">
                ...
                <form wire:submit="store" class="p-10 space-y-8">
                    ...
                    <div>
                        <label class="block text-[10px] uppercase tracking-widest font-bold text-slate-400 mb-2">Email Identity</label>
                    ...
                    <div>
                        <label class="block text-[10px] uppercase tracking-widest font-bold text-slate-400 mb-2">Access Key {{ $editingAgentId ? '(Leave blank to keep current)' : '' }}</label>
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            <label class="block text-[10px] uppercase tracking-widest font-bold text-slate-400 mb-2">Security Role</label>
                        ...
                        <div>
                            <label class="block text-[10px] uppercase tracking-widest font-bold text-slate-400 mb-2">Service Node</label>
                    ...
                    <div class="pt-4 flex justify-end space-x-4">
                        <button type="button" wire:click="closeModal" class="px-8 py-4 bg-slate-50 text-slate-500 font-bold rounded-xl hover:bg-slate-100 transition-all uppercase tracking-widest text-[10px]">Cancel</button>
