...
<div class="text-slate-900">
    <div class="mb-10">
        ...
        <p class="text-slate-500 text-sm">Please authenticate to access the workspace.</p>
    </div>
    ...
    <form wire:submit="login" class="space-y-8">
        ...
        <div class="flex items-center justify-between">
            <label for="remember" class="inline-flex items-center cursor-pointer group">
                ...
                <span class="ms-3 text-[10px] font-bold text-slate-400 group-hover:text-slate-600 uppercase tracking-widest transition-colors">{{ __('Keep me active') }}</span>
            </label>
            
            @if (Route::has('password.request'))
                <a class="text-[10px] font-bold text-slate-400 hover:text-indigo-600 transition-colors uppercase tracking-widest" href="{{ route('password.request') }}" wire:navigate>
                    {{ __('Recovery') }}
                </a>
            @endif
        </div>
