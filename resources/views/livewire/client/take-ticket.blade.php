<div class="min-h-screen bg-[#f4f6f8] flex flex-col items-center justify-center p-6 text-slate-900 font-sans">
    ...
    <div class="absolute top-8 right-8 z-20">
        <a href="{{ route('login') }}" class="group px-6 py-3 bg-white border-2 border-black rounded-2xl text-black hover:bg-slate-50 hover:shadow-lg transition-all text-xs font-bold uppercase tracking-widest flex items-center">
            Staff Portal
        </a>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            @forelse($services as $service)
                <button wire:click="takeTicket({{ $service->id }})" 
                    class="group relative flex flex-col h-full bg-white border-2 border-black rounded-[2.5rem] p-10 text-left transition-all duration-300 hover:bg-slate-50 hover:shadow-lg hover:-translate-y-1">
